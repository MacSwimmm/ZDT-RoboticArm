...
/* 定义 Keil 调试观察变量 */
volatile Emm_Debug_t g_emm_debug = {0};

/* 包装 HAL_UART_Transmit_DMA 函数以记录调试信息。
 * 商家 F407 例程要求每条指令后留 10ms 间隔，防止电机端粘包/来不及解析。
 * 因此这里把 DMA 发送包装成“等待物理发完 + 10ms 帧间隔”的可靠发送。
 */
HAL_StatusTypeDef Emm_V5_Transmit_DMA_Wrapper(UART_HandleTypeDef *huart, uint8_t *cmd, uint16_t len)
{
    uint32_t start_tick;
    g_emm_debug.last_tx_len = (len > 16) ? 16 : len;
    for (uint8_t i = 0; i < g_emm_debug.last_tx_len; i++) {
        g_emm_debug.last_tx_buf[i] = cmd[i];
    }
    
    HAL_StatusTypeDef status = HAL_UART_Transmit_DMA(huart, cmd, len);
    ...
    g_emm_debug.last_tx_status = (uint8_t)status;
    
    if (status == HAL_OK) {
        g_emm_debug.tx_success_count++;
    } else if (status == HAL_BUSY) {
        g_emm_debug.tx_busy_count++;
    } else {
        g_emm_debug.tx_error_count++;
    }
    
    g_emm_debug.uart_gstate = (uint32_t)huart->gState;
    g_emm_debug.uart_error_code = huart->ErrorCode;
    
    return status;
}

/* DMA发送完成回调函数（由 HAL 库在 DMA 传输结束中断中自动调用） */
void HAL_UART_TxCpltCallback(UART_HandleTypeDef *huart)
{
    if (huart->Instance == USART1) {
        g_emm_debug.uart_gstate = (uint32_t)huart->gState;      // 传输完成后，状态更新为 READY (0x20)
        g_emm_debug.uart_error_code = huart->ErrorCode;
    }
}

/* 重定义 HAL_UART_Transmit_DMA 宏，使得后续所有的发送均通过 Wrapper 执行 */
#define HAL_UART_Transmit_DMA(huart, cmd, len) Emm_V5_Transmit_DMA_Wrapper(huart, cmd, len)
