...
/* ===================== Keil 调试观察结构体 ===================== */
typedef struct {
    uint8_t last_tx_status;            // 上一次 DMA 发送返回值 (0:OK, 1:ERROR, 2:BUSY, 3:TIMEOUT)
    uint32_t tx_success_count;         // 发送成功（返回 HAL_OK）的次数
    uint32_t tx_busy_count;            // 发送时处于 BUSY 状态的次数
    uint32_t tx_error_count;           // 发送错误的次数
    uint32_t uart_gstate;              // huart1.gState的值，用于观察串口发送状态机是否锁死
    uint32_t uart_error_code;          // huart1.ErrorCode 错误码
    uint8_t last_tx_buf[16];           // 上一次尝试发送的数据帧内容
    uint8_t last_tx_len;               // 上一次尝试发送的数据长度
} Emm_Debug_t;

extern volatile Emm_Debug_t g_emm_debug;
/**********************************************************
***	Emm_V5.0步进闭环控制例程
***	编写作者：ZHANGDATOU
***	技术支持：张大头闭环伺服
